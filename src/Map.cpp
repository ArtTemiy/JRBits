...
namespace Engine {
    ...
    Tile& Map::GetTile(uint row, uint column) {
        return tiles_[row][column];
    }

    Tile& Map::GetTile(const TileCoordinates &coordinates) {
        return tiles_[coordinates[0]][coordinates[1]];
    }

    const Tile& Map::GetTile(uint row, uint column) const {
        return tiles_[row][column];
    }

    const Tile& Map::GetTile(const TileCoordinates &coordinates) const {
        return tiles_[coordinates[0]][coordinates[1]];
    }

    const std::vector<Tile>& Map::operator[](uint row) const {
        return tiles_[row];
    }

    std::vector<Tile>& Map::operator[](uint row) {
        return tiles_[row];
    }
    ...
    bool Map::ComputePath(const TileCoordinates &start_tile, const TileCoordinates &end_tile, Path &path) {
        ...
        while (!tiles_queue.empty()) {
            ...
            if (next_tile.is_block || visited_tiles[coordinates[0]][coordinates[1]] != kNotVisited) {
                continue;
            }
